Feature: View an object
    ...
    Scenario: Render an object with a complex label template
        When I navigate to a dataset "The Eggheads"
        And I navigate to "Current scoresheet"
    ...
    Scenario: Render properties in a list of nested objects
        ...
        Then it should display properties within an outer list "Score menu"
            ...
            | 1     | string  | Measurement | Was the build broken by new code added this sprint?                           | no   |            |
            | 2     | integer | Range       | -2, 0                                                                         | yes  | 2          |
            | 2     | string  | Measurement | Were bugs found in production (only ones introduced during the last release)? | no   |            |